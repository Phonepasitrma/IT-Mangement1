...
const AssetDetail: React.FC = () => {
  ...
  const fetchCheckLogs = async (assetId: string) => {
    try {
      const response = await api.get<CheckLog[]>(`/checklogs/asset/${assetId}`);
        ...
        <Col md={6}>
          <Card className="mb-3">
            ...
            <Card.Body>
              ...
              <Row className="mt-2">
                <Col md={4} className="fw-bold">Model:</Col>
                <Col md={8}>{asset.ModelName || asset.Brand || 'N/A'}</Col>
              </Row>
              <Row className="mt-2">
          ...
          <Card className="mb-3">
            ...
            <Card.Body>
              <Row>
                ...
                <Col md={8}>{asset.DatePurchase ? formatDate(asset.DatePurchase) : 'N/A'}</Col>
              </Row>
              <Row className="mt-2">
                <Col md={4} className="fw-bold">Price:</Col>
                <Col md={8}>${asset.Price ? asset.Price.toFixed(2) : '0.00'}</Col>
              </Row>
              <Row className="mt-2">
                <Col md={4} className="fw-bold">Replacement Cost:</Col>
                <Col md={8}>${asset.ReplacementCost ? asset.ReplacementCost.toFixed(2) : 'N/A'}</Col>
              </Row>
              <Row className="mt-2">
                <Col md={4} className="fw-bold">End of Life:</Col>
                <Col md={8}>{asset.EndOfLife ? formatDate(asset.EndOfLife) : 'N/A'}</Col>
              </Row>
            </Card.Body>
          </Card>
        </Col>
